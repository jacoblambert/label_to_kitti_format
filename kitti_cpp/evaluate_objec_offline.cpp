...
bool eval(string gt_dir, string result_dir, Mail* mail){
  ...
  printf("start to eval image 2D bounding boxes... \n");
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    if (eval_image[c]) {
      ...
      printf("2D bounding boxes detection saved ... \n");
      if(compute_aos){
        printf("  start to compute aos ... \n");
        saveAndPlotPlots(plot_dir, CLASS_NAMES[c] + "_orientation", CLASS_NAMES[c], aos, 1);
        fclose(fp_ori);
        printf("2D bounding boxes aos saved ... \n");
      }
    }
  }
  ...
  printf("start to eval bird's eye view bounding boxes... \n");
  for (int c = 0; c < NUM_CLASS; c++) {
    CLASSES cls = (CLASSES)c;
    if (eval_ground[c]) {
  ...
  printf("start to eval 3D bounding boxes... \n");
  for (int c = 0; c < NUM_CLASS; c++) {
    CLASSES cls = (CLASSES)c;
    if (eval_3d[c]) {
